...
#define CUDA_SAFE_CALL(call)                                                                    \
  do {                                                                                          \
    ...
    if (cudaSuccess != err) {                                                                   \
      LOG(ERROR) << "Cuda error in func '" << __FUNCTION__ << "' at line " << __LINE__ << " : " \
...
__device__ __forceinline__ uint64_t xor1(uint64_t a, uint64_t b) {
  uint64_t result;
...
#if USE_XOR_ASM_OPTS
// device asm for whirpool
__device__ __forceinline__ uint64_t xor3(uint64_t a, uint64_t b, uint64_t c) {
  uint64_t result;
  asm("xor.b64 %0, %2, %3;\n\t"
      "xor.b64 %0, %0, %1;\n\t"
...
#if USE_XOR_ASM_OPTS
// device asm for whirpool
__device__ __forceinline__ uint64_t xor8(uint64_t a, uint64_t b, uint64_t c, uint64_t d, uint64_t e, uint64_t f,
...
// device asm for x17
__device__ __forceinline__ uint64_t sph_t64(uint64_t x) {
#ifdef __CUDA_ARCH__
  uint64_t result;
  asm("{\n\t"
...
__device__ __forceinline__ uint2 ROL2(const uint2 a, const int offset) {
  uint2 result;
#if __CUDA_ARCH__ > 300
  if (offset >= 32) {
